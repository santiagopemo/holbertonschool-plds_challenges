...
def anagram(ana_word="holberton", n=1, ana_text="anagram.txt"):
    ...
    if not path.exists(ana_text):
        raise FileNotFoundError("can't find {}".format(ana_text))
    if n < 1:
    ...
    with open(ana_text) as f:
        ...
        if n >= len(text_list):
            raise ValueError("Maximum number of words can't be greater than number of words in the text")
        for i in range(0, len(text_list)):
            for j in range(1, n + 1):
            # temp = full_line[i: i + n]
                try:
                    temp = text_list[i:i + j]
                except:
                    continue
                aux = a[:]
                if j == 1:
